...
Future<void> main() async {
  ...
  print('分组数量: ${groups.length}');
  for (final group in groups) {
    print('  - ${group.name} (ID: ${group.id}, 颜色: 0x${group.color.toRadixString(16).padLeft(8, '0')}, 系统分组: ${group.isSystem})');
  }
  print('');
  ...
  print('筛选类型数量: ${filterTypes.length}');
  ...
  for (final ft in filterTypes) {
    ...
    if (ft.groupId == null) {
      noGroupCount++;
      print('  ⚠️  ${ft.name} - 未分配分组');
    } else {
      if (wechatTypes.contains(ft.name)) {
        wechatGroupCount++;
        print('  ✓ ${ft.name} - 分组: $groupName');
      } else if (alipayTypes.contains(ft.name)) {
        alipayGroupCount++;
        print('  ✓ ${ft.name} - 分组: $groupName');
      } else {
        print('  ℹ️  ${ft.name} - 分组: $groupName');
      }
    }
  }

  print('\n统计:');
  print('  未分配分组的筛选类型: $noGroupCount');
  print('  微信类型已正确分配: $wechatGroupCount');
  print('  支付宝类型已正确分配: $alipayGroupCount');
  print('');

  // 3. 检查分组ID映射
  print('【3. 检查分组ID】');
  final groupMap = {for (final g in groups) g.name: g.id};
  print('  微信分组ID: ${groupMap['微信']}');
  print('  支付宝分组ID: ${groupMap['支付宝']}');
  print('  自定义分组ID: ${groupMap['自定义']}');
  print('');
  ...
  if (!needsFix) {
    print('✅ 所有筛选类型都已正确分配到分组！');
    print('   颜色应该正常显示（微信绿色、支付宝蓝色）。');
  } else {
    print('⚠️  发现 $noGroupCount 个未分配分组的筛选类型！');
    print('');
    print('【5. 修复方案】');
    print('正在自动修复...');
...
Future<void> _fixDatabase(
  ...
  final stillNoGroup = updatedFilterTypes.where((ft) => ft.groupId == null).length;
  if (stillNoGroup == 0) {
    print('✅ 所有筛选类型现在都已分配到分组！');
  } else {
    print('⚠️  仍有 $stillNoGroup 个筛选类型未分配分组');
  }
}
